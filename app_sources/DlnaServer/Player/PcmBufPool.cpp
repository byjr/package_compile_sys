...
#define BUFPOOL_MAX_SIZE 60		//15s缓存
		...
		void PcmBufPool::pushPcmChunk(uint8_t *stream, unsigned int len, unsigned int offset_ms) {
			while (true) {											//这个无限循环只能通过m_streamPool.size()的改变来跳出
				pthread_mutex_lock(&m_dataLock);					//操作m_streamPool.size()的地方有popPcmChunk(),clear()
				if (m_streamPool.size() >= BUFPOOL_MAX_SIZE) {		//以及后来加上的reduceStreamPoolData()
					m_fullFlag = true;								//因此更改播放器流程的时候要注意是否会影响这几个函数的执行
					pthread_mutex_unlock(&m_dataLock);				//否则会导致后续操作卡死
					usleep(80000);
				} else {
					break;
				}
