...
ActivityMonitorSingleton::ActivityMonitorSingleton() : m_threadAlive(true) {
    ...
    deviceCommonLib::deviceTools::setThreadName(m_threadId, APP_THREAD_NAME_ACTIVITYMONITORSINGLETON);
}
...
void ActivityMonitorSingleton::updateJsonFile() {
    APP_INFO("++++++++++++++++++++++++updateJsonFile%d-----%s", m_monitorItem.m_playerStatus, m_prevWriteContent.c_str());
    pthread_mutex_lock(&m_mutex);
...
void* ActivityMonitorSingleton::threadRoutine(void *arg) {
    auto instance = (ActivityMonitorSingleton*) arg;
    instance->updateJsonFile();
    while (instance->m_threadAlive) {
        sleep(SECS_PER_MIN);
        instance->m_monitorItem.m_timestamp++;
		dbg("m_timestamp:%d",instance->m_monitorItem.m_timestamp);
        instance->updateJsonFile();
    }
