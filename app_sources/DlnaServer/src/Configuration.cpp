...
#define AUDIO_DEFAULT_RECORD_LIBRARY     "/usr/lib64/libAlsaAudioMicrophoneWrapper.so"
#else
#define MUSIC_DEFAULT_RECODER_DEVICE  "plughw:5,1"
...
#define ALERT_DEFAULT_PLAYBACK_DEVICE  "common"
#define RAPID_DEFAULT_PLAYBACK_DEVICE  "common"
#define NATTS_DEFAULT_PLAYBACK_DEVICE  "common"
#define AUDIO_DEFAULT_RECORD_LIBRARY     "/usr/lib/libAlsaAudioMicrophoneWrapper.so"
#endif
...
Configuration::Configuration():m_appConfigFile("./appresources/app_config.json"),
                               ...
                               m_deviceId(""),
                               m_ssidPrefix(""),
                               m_recordAudioPath(MUSIC_DEFAULT_RECODER_DEVICE),		
                               m_debug(false) {
}
...
//                }else {
//                    m_debug = DCSAPP_DEBUGE_DEFAULT_CONFIG;
//                }

                if (readDocument.HasMember("audio_lib_name")) {
                    m_audioLibName = readDocument["audio_lib_name"].GetString();
                    if (m_audioLibName.empty()) {
                        m_audioLibName = AUDIO_DEFAULT_RECORD_LIBRARY;
                    }
                } else {
                    m_audioLibName = AUDIO_DEFAULT_RECORD_LIBRARY;
                }
...
#if 1
	char buf[128]="";
	cdb_get_str("$al_deviceId",buf,sizeof(buf)," ");
	if(!strncmp(buf,"CMCC-500537-",strlen("CMCC-500537-"))){
		m_deviceId = buf;
		m_isDeviceidValid = true;
	}else{
		err("devid get failed!");
		m_deviceId = deviceCommonLib::deviceTools::randomUID();
		m_isDeviceidValid = false;
		err("al_deviceId-------false-----------------:%s",buf);
	}	 
	m_commonVolume = get_mixer_volume("Common");
	m_alertsVolume = get_mixer_volume("Common");
#else
    ...
    if (access(m_duerosConfigFile.c_str(), F_OK) == 0) {
        ...
        if (duerosConfigStream.good()) {
            ...
            if (readDocument.HasParseError()) {
                ...
                saveToDuerosConfigFile(isRecreateDuerosConfigFile);
            }
        } else {
            isRecreateDuerosConfigFile = true;
        }
...
bool Configuration::isDeviceIdValid() const {
    return m_isDeviceidValid;
}
bool Configuration::getDebug() const {
    return m_debug;
}

bool Configuration::setDebug(bool en) {
    m_debug = en;
}
...
void Configuration::setCommVol(unsigned int volume) {
	...
	cdb_set_int("$ra_vol",m_commonVolume);
//    saveToDuerosConfigFile(false);
}
...
void Configuration::saveToDuerosConfigFile(bool isRecreate) {
    rapidjson::Document document;
    document.SetObject();
