...
void DeviceIoWrapper::callback(DeviceInput event, void *data, int len) {
    ...
    switch (event) {
        ...
        case DeviceInput::KEY_MIC_MUTE: {
            //handleSleepMode();
            if(DeviceIoWrapper::m_isAlertRing){
				closeLocalActiveAlert();
			    break;
			}
			if(cdb_get_int("$microphone_on",1)==1){
				SoundController::getInstance()->microphoneOff();
				m_applicationManager->microphoneOff();
			}else{
	            SoundController::getInstance()->microphoneOn();
				m_applicationManager->microphoneOn();
			}
            break;
        }
		case DeviceInput::KEY_MIC_SYNC: {
			if(cdb_get_int("$microphone_on",0)==0){
				m_applicationManager->m_micWrapper->stopStreamingMicrophoneData();
				led_close_MIC(NULL);
//				SoundController::getInstance()->microphoneOff();
			}else{
//				SoundController::getInstance()->microphoneOn();
				m_applicationManager->m_micWrapper->startStreamingMicrophoneData();
				led_open_MIC(NULL);
			}
			break;
		}
		case DeviceInput::KEY_MODE_SYNC: {
        ...
        case DeviceInput::KEY_ENTER_AP:
            ...
            break;
		case DeviceInput::OMNICONFIG_FAILED:{
				if(!m_isTouchStartNetworkConfig){
					break;
				}
				if(!data){
        ...
        case DeviceInput::KEY_PLAYBACK_NEXT: {
			m_applicationManager->PlaybackNextPressed();
            APP_INFO("DeviceIoWrapper callback KEY_PLAYBACK_PREV");
            break;
        }
        case DeviceInput::KEY_PLAYBACK_PREV: {
			m_applicationManager->PlaybackPrevious();
            APP_INFO("DeviceIoWrapper callback KEY_PLAYBACK_PREV");
            break;
        }
...
void DeviceIoWrapper::led_first_poweron(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::FIRST_POWERON);
}
void DeviceIoWrapper::led_connect_start(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::CONNECT_START);
}
void DeviceIoWrapper::led_connect_cancle(void * args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::CONNECT_CANCLE);
}
void DeviceIoWrapper::led_connecting(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::CONNECTING);
}
void DeviceIoWrapper::led_connect_succeed(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::CONNECT_SUCCEED);
}
void DeviceIoWrapper::led_wrong_password(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::WRONG_PASSWORD);
}
void DeviceIoWrapper::led_unsupported_route(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::UNSUPPORTED_ROUTE);
}
void DeviceIoWrapper::led_connection_routing_failed(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::CONNECTION_ROUTING_FAILED);
}
void DeviceIoWrapper::led_on_logout(void * args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::ON_LOGOUT);
}
void DeviceIoWrapper::led_device_boot(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::DEVICE_BOOT);
}
void DeviceIoWrapper::led_device_reboot(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::DEVICE_REBOOT);
}
void DeviceIoWrapper::led_factory_set(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::FACTORY_SET);
}
void DeviceIoWrapper::led_turn_down_the_volume(void* args){
//	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::TURN_DOWN_THE_VOLUME);
}
void DeviceIoWrapper::led_pardon_again(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::PARDON_AGAIN);
}
void DeviceIoWrapper::led_network_fault(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::NETWORK_FAULT);
}
void DeviceIoWrapper::led_binding_success(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::BINDING_SUCCESS);
}
void DeviceIoWrapper::led_network_exception(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::NETWORK_EXCEPTION);
}
void DeviceIoWrapper::led_unbind_success(void *args){
//	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::UNBIND_SUCCESS);
}
void DeviceIoWrapper::led_slow_network(void* args){
//	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::SLOW_NETWORK);
}
void DeviceIoWrapper::led_login_failure(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::LOGIN_FAILURE);
}
void DeviceIoWrapper::led_wait_moment(void* args){
//	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::WAIT_MOMENT);
}
void DeviceIoWrapper::led_idel(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::IDEL);
}
void DeviceIoWrapper::led_wakeup(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::WAKEUP);
}
void DeviceIoWrapper::led_listening(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::LISTENING);
}
void DeviceIoWrapper::led_thinking(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::THINKING);
}
void DeviceIoWrapper::led_talking(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::TTS_TALKING);
}
void DeviceIoWrapper::led_play_listen(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::PLAY_LISTEN);
}
void DeviceIoWrapper::led_play_silence(void* args){
//	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::PLAY_SILENCE);
}
...
//	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::ENTER_CONFIG_NET_MODE);
}
void DeviceIoWrapper::led_close_MIC(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::IS_MUTE);
}
void DeviceIoWrapper::led_open_MIC(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::IS_UNMUTE);
}

void DeviceIoWrapper::led_bt_mode(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::BT_MODE);
}

void DeviceIoWrapper::led_wifi_mode(void* args){
//	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::WIFI_MODE);
}


void DeviceIoWrapper::led_tts_finish(void* args){
	user_fifo_write_fmt(uartdFifoPtr,"duerLedSet %03u",(unsigned int)duerScene_t::TTS_PLAY_FINISH);
}
...
void DeviceIoWrapper::initCommonVolume(int volume) {
    DeviceIo::getInstance()->setVolume(volume,
						(0xFFFF & ~ALERT_PLAYBACK_TRACK_ID));
    volumeChanged();
}
...
//    DeviceIo::getInstance()->controlBt(BtControl::BT_RESUME_PLAY);
	DeviceIo::getInstance()->controlBt(BtControl::BT_MUSIC_NEXT);
}

void DeviceIoWrapper::btPlayNext() {inf(__func__);
...
void DeviceIoWrapper::handlePlayAndPause() {
    if (!isPauseBtnClicked()) {
        ...
        if (m_applicationManager) {
            m_applicationManager->forceHoldFocus(true);
            m_applicationManager->playPauseButtonPressed(m_pauseBtnClicked);
        }
    } else {
