...
#define AVIO_CTX_BUFFER_SIZE 5120
...
Mp3TtsPlayerImpl::Mp3TtsPlayerImpl(const std::string& audio_dev) : m_executor(nullptr),
    ...
    deviceCommonLib::deviceTools::setThreadName(m_playThread, "PLY_ttsplay");
}
    ...
    while (player->m_aliveFlag) {
        ...
        while (true) {            
            ...
            if (ret >= 0) {
                ...
                if (error_code == 0) {
					memcpy(player->m_pcmResampleBuf, frame->data[0], frame->nb_samples*2);
        ...
        av_frame_free(&frame);
#ifndef FIXED_RESAMPLING_PARAMETERS
