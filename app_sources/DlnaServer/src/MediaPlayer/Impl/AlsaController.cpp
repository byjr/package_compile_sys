...
void AlsaController::writeStream(unsigned int channels,
	...
	int c = buff_size * 8 / m_bits_per_frame;
	if (c < chunk_size) {
		if (m_abortFlag) return;
	...
	while ( c > 0) {
		...
		if (r == -EAGAIN || (r >= 0 && (size_t)r < c)) {
			...
			war("failed to snd_pcm_writei: %s", snd_strerror(r));
			break;
		}
		if (r > 0) {
			c -= r;
...
bool AlsaController::alsaClear1() {
    AutoLock autoLock(m_lock);
    if (nullptr != m_pcmHandle) {
