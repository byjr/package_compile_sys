...
static unsigned char audio_buf[BYTES_PER_SAMPLE * ASP_SAMPLES_PER_READ] __DCACHE_ALIGNED__;
#endif
...
#if defined(CONFIG_PANTHER)
void ASP_acquire_data(int *samples, struct recorder* rec)
{
    snd_pcm_t* handle = (snd_pcm_t*)rec->wavein_hdl;
again:
    int frames = snd_pcm_readi(handle, rec->audiobuf, ASP_SAMPLES_PER_READ);
...
void* record_thread_proc(void* para) {
    ...
    while (rec->running) {
        ASP_acquire_data(&samples, rec);

        if(samples < 0) {
            break;
        }
        rec->on_data_ind(rec->audiobuf, samples * BYTES_PER_SAMPLE, rec->userdata);
    }
...
int AudioRecorder::recorderOpen(const char* alsa_rec_device,
    ...
    do {
        ...
        pthread_setname_np(_m_rec->rec_thread, "RECAlsaRecorder");
        return 0;
    } while (0);
