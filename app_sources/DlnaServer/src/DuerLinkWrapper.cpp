...
void DuerLinkWrapper::verifyNetworkStatus(bool wakeupTrigger) {inf(__func__);
...
//          DeviceIoWrapper::getInstance()->ledNetOff();
			...
			if(!strncmp(buf,"CMCC-31222-",strlen("CMCC-31222-"))){//deviceid 已经拿到（未解绑）
				if((access("/tmp/aTokenInfo.txt",F_OK))!=-1) {//aToken 已经拿到
					cdb_set_int("$op_work_mode",9);//尝试回连
					vfexec("mtc commit",0);
					setNetworkStatus(DUERLINK_NETWORK_CONFIG_SUCCEEDED);
				}
			}
			cdb_set_int("$device_online",1);
...
void DuerLinkWrapper::wakeupDuerLinkNetworkStatus() {
    switch (m_networkStatus) {
        case DUERLINK_NETWORK_CONFIG_STARTED: {
			...
			inf("wakeup_duer_link_network_status: DUERLINK_NETWORK_CONFIG_STARTED=====");
//            DeviceIoWrapper::getInstance()->ledNetWaitConnect();
            SoundController::getInstance()->linkStart();
            break;
        }
        case DUERLINK_NETWORK_CONFIGING: {
			...
			inf("wakeup_duer_link_network_status: DUERLINK_NETWORK_CONFIGING=====");
//            DeviceIoWrapper::getInstance()->ledNetDoConnect();
            SoundController::getInstance()->linkConnecting();
            break;
        }
        case DUERLINK_NETWORK_RECOVERY_START: {
			...
			inf("wakeup_duer_link_network_status: DUERLINK_NETWORK_RECOVERY_START=====");
//            DeviceIoWrapper::getInstance()->ledNetRecoveryConnect();
            SoundController::getInstance()->reLink();
            break;
        }
        case DUERLINK_NETWORK_FAILED: {
			...
			inf("wakeup_duer_link_network_status: DUERLINK_NETWORK_FAILED=====");
//            DeviceIoWrapper::getInstance()->ledNetConnectFailed();
        ...
        case DUERLINK_NETWORK_RECOVERY_FAILED: {
			...
			inf("wakeup_duer_link_network_status: DUERLINK_NETWORK_RECOVERY_FAILED=====");
//            DeviceIoWrapper::getInstance()->ledNetConnectFailed();
            SoundController::getInstance()->reLinkFailed();
            break;
        }
...
void DuerLinkWrapper::registerRfreshAccessToken(set_refresh_access_token_t getAccessToken){
//	duerLinkSdk::get_instance()->registerRfreshAccessToken(getAccessToken);
}
