...
//    explicit ApplicationManager() = default;
    ...
    /*
     * Implement onListenStarted here
     */
    void onListenStarted(bool success) override;

    /*
     * Implement onStopListen here
     */
	...
	bool globalGetAccessToken(std::string& accessToken);
	void setGetAccessTokenType(size_t type);
    /**
     * get access token.
     * @param message
     */
    bool getAccessToken(std::string& accessToken) override;
    ...
    */
    ...
    /**
     * Inform the server that play/pause button pressed
     */
    void playPauseButtonPressed(bool pausePressed);

	void PlaybackNextPressed(void);

	void PlaybackPrevious(void);
...
private:
	...
	volatile size_t GetAccessTokenType;
	...
	volatile bool m_isDialogValid;
	static ApplicationManager * ApplicationManagerPtr;
	std::mutex aTokenMtx;
};
