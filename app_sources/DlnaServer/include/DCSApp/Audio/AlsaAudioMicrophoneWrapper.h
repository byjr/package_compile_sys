...
public:
     ...
     */
    static std::unique_ptr <AlsaAudioMicroponeWrapper> create(
    ...
    void excuteWakeupCheck(void);
	void excuteExpectSpeechResponse(void) override;
	void excuteWakeupResponse(void) override;
	void checkAndStopMutiVad(void) override;
...
//    void setListenStarted(bool start) override;
//#endif
	void cmccVoipRcordingstarted(void) override;
	void cmccVoipRcordingStoped(void) override;
...
private:
	...
	static void * uploadThreadRoutine(void *args);
	static void * wakeupThreadRoutine(void *args);
...
//    AudioRecorder* _m_rec;
     ...
     */
    std::mutex m_mutex;
	pthread_t wakeupThreadId;
	pthread_t uploadThreadId;
