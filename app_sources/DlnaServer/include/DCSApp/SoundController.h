...
#include <MediaPlayer/Impl/PcmFilePlayerImpl.h>
#include <pthread.h>
#include <string>
...
public:
	...
	void userTipSoundPlay(const std::string &url,int type,bool focus);
	...
	static void linkStartedCallback(void *arg);
	static void linkFinshedCallback();
	static void	linkExitFinishedCallback();
	void exTtsPlay(bool foucs);
	void forceHoldFocusEnter(void *args);
	void forceHoldFocusExit();

	void AudioCallErrTip();
	...
	volatile bool relink_sound;

	void setRelinkSound(bool sound){
		relink_sound = sound;
	}
...
private:
     ...
     * \endif
     */
    static mediaPlayer::PcmFilePlayerImpl *m_pcmPlayer;
    ...
    unsigned long m_lastPlayTimestamp;
#else
    int64_t m_lastPlayTimestamp;
#endif
