...
public:
    Mp3FilePlayerImpl(const std::string& alertsAudioDev, const std::string& promptAudioDev);

    ~Mp3FilePlayerImpl();

    void registerListener(std::shared_ptr<AudioPlayerListener> notify) override;

    void registerListener(std::shared_ptr<AudioPlayerListener> notify, FilePlayType type) override;

    void setFilePlayType(FilePlayType type) override;
    ...
    static std::shared_ptr<Mp3FilePlayerImpl> create(const std::string& alertsAudioDev, const std::string& promptAudioDev);
...
private:
    ...
    AlsaController* m_alertsAlsa;
    AlsaController* m_promptAlsa;
    AudioDecoder* m_audioDecoder;
    std::shared_ptr<AudioPlayerListener> m_alertsListener;
    std::shared_ptr<AudioPlayerListener> m_promptListener;
    std::shared_ptr<AudioPlayerListener> m_currentListener;
    uint8_t* m_pcmBuffer;
