...
#define DATA_UNIT_MAX_BYTES (121*100)
	...
	struct AppManagerPar {
		std::shared_ptr<cppUtils::One2Multi> o2m;
...
int One2Multi_main(int argc, char *argv[]) {
	...
	if(oParListPtr->size() <= 0) {
		s_err("oParListPtr == 0");
		return -1;
	}
	if(iParListPtr->size() != 1) {
		s_err("iParListPtr != 1");
		return -1;
	}
	auto main_loop = [&oParListPtr, &iParListPtr]()->int{
		//==============启动数据输出任务模块===================
		...
		par->mtxPtr = iMtxPtr;
		auto ptr = std::make_shared<cppUtils::One2Multi>(par);
		if(!(ptr && ptr->isReady())) {
