...
std::unordered_map<int,const char*> epEvtMap = {
	{EPOLLIN,"EPOLLIN"},
	{EPOLLOUT,"EPOLLOUT"},
		...
		for(int i=0;i<nReadys;i++){
			...
			if(fd == mSocket){
				...
				if(0){
					...
					auto hd = mThMap[fd];
					if(hd){s_inf("got %d %s event!",fd,epEvtMap[EPOLLIN]);
						hd->notifyReadAble();
					}
				}else if(events[i].events == EPOLLOUT){
					auto hd = mThMap[fd];
					if(hd){s_inf("got %d %s event!",fd,epEvtMap[EPOLLOUT]);
						hd->notifyWriteAble();
					}						
				}else if(events[i].events == EPOLLRDHUP){
					//Stream socket peer closed connection, or shut down writing half of connection.
					auto hd = mThMap[fd];
					if(hd){s_inf("got %d %s event!",fd,epEvtMap[EPOLLRDHUP]);
						hd->notifyPeerClosed();
					}					
				}else if(events[i].events == EPOLLPRI){s_inf(__func__);
					s_inf("got %d %s event,un handle!",fd,epEvtMap[EPOLLPRI]);					
					// There is urgent data available for read(2) operations.
				}else if(events[i].events == EPOLLERR){s_inf(__func__);
					...
					s_inf("got %d %s event,un handle!",fd,epEvtMap[EPOLLERR]);
					...
					s_inf("got %d %s event,un handle!",fd,epEvtMap[EPOLLHUP]);
				}else if(events[i].events == EPOLLWAKEUP){	s_inf(__func__);
						...
						s_inf("got %d %s event,un handle!",fd,epEvtMap[EPOLLWAKEUP]);
				}else{
					s_inf("got %d 's event:%d, un handle!",fd,events[i].events);
					auto hd = mThMap[fd];
					if(hd){
						hd->notifyPeerClosed();
					}
