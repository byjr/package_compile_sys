...
#define WRITE_EVT_FLAG (EPOLLOUT|EPOLLET)
...
bool TaskHandler::doHandle(){
	if(mMethod == "GET"){
		...
		if(mCommand == ""){
			if(EpollWrapper::mod_evt(mPar->epfd,mPar->fd,WRITE_EVT_FLAG) == false){
...
bool TaskHandler::run(){
	...
	do{
		...
		succeed = getContex();
		if(!succeed){			
			responseText("ERR:server got a Incorrect Contex!");
			break;
		}
		succeed = doFilter();
		if(!succeed){
			responseText("ERR:the request was filtered by server!");
			break;
		}
		succeed = doHandle();
		if(!succeed){
			responseText("ERR:the request doHandle failed!");
			break;
		}
		succeed = true;
	}while(0);
...
TaskHandler::TaskHandler(std::weak_ptr<TaskHandlerPar> par){
	...
	if(EpollWrapper::add_evt(mPar->epfd,mPar->fd,EPOLLIN|EPOLLET) == false){
...
TaskHandler::~TaskHandler(){
	...
	if(EpollWrapper::del_evt(mPar->epfd,mPar->fd,EPOLLOUT|EPOLLET) == false){
		s_err("EpollWrapper::add_evt failed!");
		return;
	}
	close(mPar->fd);
