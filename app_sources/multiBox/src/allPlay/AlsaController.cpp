...
#define ALSA_MAX_BUFFER_TIME 500000
		...
		bool AlsaController::setParams(unsigned int rate, unsigned int channels) {
			...
			allPlayerInf("chunBytes = %d, chunk_size = %d, bits_per_frame=%d\n", m_chunkBytes, chunk_size, m_bits_per_frame );
			return true;
		}
		...
		void AlsaController::writeStream(unsigned int channels,
			...
			while ( c > 0) {
				...
				if (r == -EAGAIN || (r >= 0 && (size_t)r < c)) {
					...
					allPlayerWar("EPIPE, Buffer Underrun.");
					if ( (r > 0) && (r <= c) ) {
						if (m_abortFlag) break;
					...
					if (r < 0) {
						...
						if ((r = snd_pcm_prepare(m_pcmHandle)) < 0) {
							allPlayerWar("alsa resume: pcm_prepare %s", snd_strerror(r));
							break;
						}
						continue;
					}
				} else if (r < 0) {
					allPlayerWar("failed to snd_pcm_writei: %s", snd_strerror(r));
					break;
				}
				if (r > 0) {
					c -= r;
